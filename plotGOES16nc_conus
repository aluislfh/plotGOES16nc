...
def main():
    ruta = "/home/adrian/aSispi/GOES16"
    os.chdir(ruta)
...
#-------------------------------------------------------------------------------
    ...
    # GOES 16 - Channel 14
    lista = glob.glob("OR_ABI-L2-CMIPC-M6C14_G16_s*.nc")  # OR_ABI-L2-CMIPC-M6C14_G16_s20192040701394_e20192040704167_c20192040704224.nc
    lista.sort()
...
#    # GOES 16 - Channel 14
#    lista = glob.glob("OR_ABI-L2-CMIPM1-M3C10_G16_*.nc")
#    lista.sort()
...
#    # GOES 16 - Channel 14
#    lista = glob.glob("OR_ABI-L2-CMIPM1-M3C09_G16_*.nc")
#    lista.sort()
...
#    # GOES 16 - Channel 14
#    lista = glob.glob("OR_ABI-L2-CMIPM1-M3C08_G16_*.nc")
#    lista.sort()
...
#    path="OR_ABI-L2-CMIPM1-M3C14_G16_s20190280235234_e20190280235291_c20190280235337.nc"
    ...
    nctest = Dataset('/home/adrian/aSispi/wrfout_d03_2019-08-02_00:00:00')  #cualquier corrida
    lons =  nctest.variables['XLONG'][0,:]
    lats =  nctest.variables['XLAT'][0,:]
    nctest.close()
    ...
    for i in range(lons.shape[1]):
        for j in range(lats.shape[0]):
            if lons[-1,i]>=np.min(lons) and lats[j,-1]>=np.min(lats) and lons[-1,i]<=np.max(lons) and lats[j,-1]<=np.max(lats):
                ii.append(i)
                jj.append(j)
    ...
    bmap = Basemap(projection='cyl', llcrnrlon=lons[-1,jmin], llcrnrlat=lats[imax,-1], urcrnrlon=lons[-1,jmax], urcrnrlat=lats[imin,-1],  resolution='l')
    ...
    plt.savefig('G16_C' + str(Band) + '_' + date_save + '_' + time_save + '.png', dpi=DPI, bbox_inches='tight', pad_inches=0)
    plt.clf()
    plt.cla()
