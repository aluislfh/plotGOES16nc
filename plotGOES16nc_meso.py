...
#-------------------------------------------------------------------------------
    ...
    # GOES 16 - Channel 14
    lista = glob.glob("OR_ABI-L2-CMIPM1-M3C14_G16_*.nc")
    lista.sort()
...
#    path="OR_ABI-L2-CMIPM1-M3C14_G16_s20190280235234_e20190280235291_c20190280235337.nc"
    ...
    for i in range(lons.shape[1]):
        for j in range(lats.shape[0]):
            if lons[-1,i]>=-83.2 and lats[j,-1]>=22.3 and lons[-1,i]<=-81.4 and lats[j,-1]<=23.7:
                ii.append(i)
                jj.append(j)
    ...
    bmap = Basemap(projection='cyl', llcrnrlon=lons[-1,jmin], llcrnrlat=lats[imax,-1], urcrnrlon=lons[-1,jmax], urcrnrlat=lats[imin,-1],  resolution='h')
...
#    bmap.readshapefile('Nueva_DPA','Nueva_DPA',linewidth=0.90,color='darkslategray')
    ...
    plt.savefig('G16_C' + str(Band) + '_' + date_save + '_' + time_save + '_meso.png', dpi=DPI, bbox_inches='tight', pad_inches=0)
    plt.clf()
    plt.cla()
